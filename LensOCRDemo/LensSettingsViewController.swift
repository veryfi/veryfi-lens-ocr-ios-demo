...
class LensSettingsViewController: UIViewController {
    ...
    let imageProcessingSection: [(Keys,Types)]  = [(.autoRotateIsOn, .switchCell),(.autoDocDetectionAndCropIsOn,.switchCell),(.blurDetectionIsOn, .switchCell),(.autoSkewCorrectionIsOn,.switchCell),(.autoCropGalleryIsOn, .switchCell), (.ocrRegex,.stringValueCell)]
    let uiSection: [(Keys,Types)]  = [(.docDetectFillUIColor, .stringColorCell), (.submitButtonBackgroundColor, .stringColorCell),(.submitButtonBorderColor,.stringColorCell),(.submitButtonFontColor,.stringColorCell),(.docDetectStrokeUIColor,.stringColorCell),(.submitButtonCornerRadius, .integerValueCell),(.manualCropIsOn,.switchCell),(.moreMenuIsOn,.switchCell),(.moreSettingsMenuIsOn,.switchCell),(.galleryIsOn,.switchCell),(.dictateIsOn,.switchCell),(.emailCCIsOn,.switchCell),(.emailCCDomain,.stringValueCell),(.rotateDocIsOn,.switchCell),(.shieldProtectionIsOn,.switchCell),(.ocrViewWidth,.integerValueCell), (.ocrViewHeight,.integerValueCell), (.ocrViewCornerRadius,.integerValueCell)]
    ...
    override func viewDidLoad() {
        ...
        settings.ocrRegex = "[A-Z0-9]{10}" //Prefilled regex for easier use, it can also be modified on app running real time
